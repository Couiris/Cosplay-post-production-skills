...
description: >
  为 COS 场馆照片生成可直接导入插件并用于 Nano Banana/Gemini 图像编辑的半合成两阶段 JSON 预设。
  当用户要求锁定人物、动作、位置、比例、画幅及场馆远墙和天花板，同时依据角色背景与性格加入可控的
  地面、前景、中景、远景、原景保留区和浮空元素时使用。覆盖古风、和风、校园、咖啡厅、废墟、日式
  街景、森林、欧式、战场、赛博朋克、科技、雪景、樱花季、甜品店、魔法工坊、中式园林、舞台、
  中式古风房间、中式庭院、欧式贵族房间和沙滩等场照半合成；也用于清除路人、摄影器材、主角及人物影子，
  按原场馆材质生成无人背景，并在保持原画框 1:1 不变时按需向外扩图。不用于彻底换背景或重新摆放人物的大合成。
---
...
唯一交付是两份按顺序执行、可直接导入插件的预设 JSON：`step1_subject_shadow_composite` 与 `step2_subject_removed_refined_plate`。这里的“半合成”是让新增内容包裹原照片并始终保留场馆墙顶结构，而非把会场替换成新世界。阶段 1 生成基础布景板与独立人物影子层，由宿主按绝对坐标回贴原始人物；阶段 2 以阶段 1 为不可变底图，只输出人物/影子移除补丁及透明增材、材质精修和特效层，再由宿主确定性合成。每次先读取 [照片保真、清场与可选扩图合同](references/photo-fidelity-cleanup.md)，再读取 [半合成两阶段流程](references/two-stage-workflow.md)。
...
9. 按 [半合成两阶段流程](references/two-stage-workflow.md) 生成两份预设。阶段 1 固定为 1A 清场后的基础布景板、1B 独立影子层、1C 宿主回贴原人物；阶段 2 固定为 2A 移除主角及其影子并局部补洞、2B 透明增材/精修/特效层、2C 宿主确定性合成。任何模型全画幅阶段 2 输出都丢弃。
...
- 不可以：替换或重绘背景墙与天花板；整幅换成无遮挡的新世界；改变人物光色迁就背景；让前景大片遮住人物；改变头发、披风或裙摆方向制造风感。
- 室外主题（森林、沙滩、街景等）也必须解释成场馆内主题布景、滨海/植物展馆或舞台景片，明确保留远墙与天花板。
- 若请求实际是完全换景、大规模重构或海报级合成，说明超出半合成边界，改用 `$cos-large-composite-prompt`；两者都采用两阶段执行，但半合成额外永久锁定原场馆墙顶结构。
...
- 阶段 1 id 使用 `f_scene_<slug>-step1-subject-shadow-composite`；阶段 2 id 使用 `f_scene_<slug>-step2-subject-removed-refined-plate`。两份 `category` 固定为 `scene`，`subCategory` 使用场景中文名，`refImages` 默认空数组，`_isFactory` 为 true。
...
- 阶段 1 的人物像素只能来自宿主原始透明抠图回贴；阶段 2 只能生成显式遮罩内的 RGBA 补丁/叠加层。执行器不能输出透明层或不能按画布绝对坐标确定性合成时必须停止并报告，不能退回模型整图重绘。
