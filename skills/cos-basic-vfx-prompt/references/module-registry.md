...
| `enhance_plan` | 普通去模糊、明确超分与已有细节恢复；暗光彩噪由 `low_light_rescue` 负责 | 1 |
...
| `edge_halo_spill_fix` | 修复抠图亮边、色溢、绿幕污染和半透明边缘接缝 | 2 |
...
| `capture_match_plan` | 统一新增/修补区域与底图的噪点、锐度、色边、压缩和镜头痕迹 | 1 |
...
| `output_finish_plan` | 明确输出尺寸、色彩空间、位深、格式和克制的输出锐化 | 1 |

共 61 个规范模块。库中没有的新需求可新增临时模块，但必须遵循五段式、锁定合同和授权规则；若会长期复用，再补入本表和对应模块库。
