...
description: >
  为 COS 人像生成完全换世界、扩画幅、海报级重构、巨型资产与复杂分层 VFX 的两阶段 JSON 预设。人物由宿主原抠图绝对坐标回贴，影子为独立层，阶段 2 只输出无人无影补洞与透明叠加层；保留场馆墙顶时改用半合成，局部后期改用基础 VFX skill。
---

# COS 大合成提示词

交付两份按顺序使用的插件预设：`step1_subject_shadow_composite` 与 `step2_subject_removed_refined_plate`。阶段 1 只生成背景板和独立影子层，最终人物由宿主把原始透明抠图按绝对坐标确定性回贴；阶段 2 锁定底图坐标、移除人物及其影子、补全遮挡区，增加第二批素材，把阶段 1 素材细化为最终质量并添加背景特效。先读取 [请求路由、能力门与质量门](references/request-routing-qc.md)。
...
- 两份都必须含相同 `pair_id`、兼容的 `pair_manifest` 与逐字段一致的 `executor_profile`；能力门未通过时只输出阻断诊断，不得伪称可执行。
...
先编译 `intent_card`，建立执行器能力档案并通过硬能力门。用户明确指定的世界、保留项、禁止项、参考图职责和交付格式优先于默认场景库；缺少人物抠图、RGBA patch/layer、绝对坐标宿主合成或像素差分时停止可执行流程。
...
5. 阶段 1 固定拆成 1A 拍摄角度锁定与背景板、1B 独立影子层、1C 宿主确定性合成。先以原片地平线、机位高度、pitch/yaw/roll、人物视线高度、脚点透视、两组环境直线和身体透视缩短复原真实拍摄角度；1A 再只在人物 alpha 外生成服从该角度的场景骨架与五层基础素材；1B 以原人物作为光照/承重点参考，只输出画布同尺寸透明影子层；1C 从背景板复制开始，先合成影子层，再按原始人物 alpha 逐像素回贴人物。模型不得返回或采用重绘、移动、缩放后的完整人物。完成后输出 `subject_lock_contract`、`stage1_execution_contract`、`shadow_lock_package` 和 `base_frame_manifest`。
...
- `subject_lock_contract.subjects[]` 必须逐人声明原始透明人物引用、画布绝对坐标、alpha/bbox、中心、头顶、承重点、宽高、旋转、像素哈希和遮挡顺序；`union_subject_alpha` 供阶段 2 移除全部人物。`subject_transform_allowed: false`、`subject_repaint_allowed: false`、`subject_model_output_allowed: false`。
- `stage1_execution_contract` 固定为：1A 生成背景板（人物区 alpha=0 或由宿主遮挡）；1B 生成透明影子层（人物像素 alpha=0）；1C `output = copy(background_plate_rgba)`，先按绝对坐标合成影子，再按原始人物 alpha 逐像素回贴人物。任何模型整图人物都丢弃。
...
- 2A 只生成 `subject_fill_patch_rgba`：尺寸与画布相同或携带绝对坐标，`subject_removal_mask OR shadow_removal_mask` 外 alpha 必须为 0；不得返回或采用全画幅背景。
...
- 若交付还需要可见人物成片，使用 `final_assembly_manifest` 在无人无影底板之上按 `["background_plate_rgba","shadow_layer_rgba","original_subjects","subject_relight_overlay_rgba","foreground_occluder_rgba"]` 装配；影子层、人物染光覆盖层和前景遮挡层均可为空，但人物源层必须逐通道不变。
...
- `authorized_change_mask = (subject_removal_mask OR shadow_removal_mask OR asset_addition_mask OR material_refinement_mask OR background_vfx_mask)`。通常新增层避开人物/影子移除区；若背景侧纹理或 VFX 需要在已验收补洞区连续延伸，必须使用独立显式 mask、从完整背景逻辑生成并验证无人物形负空间，不能沿人物轮廓排布。
...
丰富度按 `composition_budget` 自适应，不把五层同时默认拉高：用户未要求的层可关闭；通常只保留 1 个主视觉、2–4 个辅助素材族和最多 2 个明显自发光源。启用层的 detail_precision 可默认 75，但最终可见度必须服从景深与空气透视。
...
- 每个特效输出 `anti_sticker_contract` 与 `anti_sticker_validation`，用体积/厚度、前后穿插、边缘衰减、独立人物染光覆盖层、投影/反射、介质/接触反应及颗粒匹配消除贴图感；任一关键项缺失即拒绝。
...
- 阶段 1 只在真实接触面生成接触影；投射影从阶段 1 实际背景光源反推，并按光源方向、高度、尺寸、颜色、人物姿态、遮挡和表面起伏生成。悬空或无可见承载面时允许 `shadow_mode:none`，但必须写原因，不伪造脚底影；多人影子记录 caster IDs。阶段 2 必须读取这些影子的差分区域并移除。
...
- 阶段 2 的 `stage2_asset_expansion` 可以在空区增加第二批辅助道具、植物、结构附件和空间层次素材，但不得替换或移动阶段 1 主资产。默认避开人物/影子移除区；2A 补洞验收后，确需连续延伸的背景纹理或 VFX 才能用独立显式 mask 进入该区，并验证不存在人形负空间。
